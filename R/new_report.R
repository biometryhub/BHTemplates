...
#' @param type The type of the report. Valid types are `biometry_hub_report` (or just `bh_report`), `html_presentation`, `short_report`, `knitr_report` and `powerpoint_presentation`. Partial matching is performed.
#'
...
knitr_report <- function(filename) {
  ...
  files <- list.files(file.path(find.package("BHTemplates"),
